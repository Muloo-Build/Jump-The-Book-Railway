...
- `components/book-metadata.tsx` — On mount, calls `searchAndFetchWork(title, author)` (Open Library `/search.json` then `/works/{key}.json`) to enrich demo + searched books with description, subjects, first sentence. localStorage-cached for 7 days, hashed key (djb2 over `title|author`) so non-ASCII titles don't collide.
