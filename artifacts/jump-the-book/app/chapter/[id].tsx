...
export default function ChapterScreen() {
  ...
  const { generate, isLoading: aiLoading } = useGenerateScene();
  ...
  // Progressive unlock: show only scenes up to user's position in this chapter
  const allScenes = chapter?.scenes ?? [];
  const unlockedScenes = savedPos
    ? allScenes.filter((_, i) => {
        const scenePct = ((i + 1) / allScenes.length) * 100;
        const chapterUnlocked = savedPos.chapter > (chapter?.chapterNumber ?? 1)
          ? 100
          : savedPos.chapter < (chapter?.chapterNumber ?? 1)
          ? 0
          : savedPos.percentComplete;
        return scenePct <= Math.max(chapterUnlocked, 25); // always show at least 1 scene
      })
    : allScenes;
  ...
  const allLocations = [...new Set(visibleScenes.map((s) => s.location))];
  const allMoods = [...new Set(visibleScenes.map((s) => s.mood.split(",")[0].trim()))];

  // Auto-generate AI scenes for user-added books (no pre-baked scenes) or on demand
  const canGenerateAI = book !== undefined;
  ...
  const handleGenerateAI = async () => {
    ...
    const res = await generate({
      ...
      spoilerMode: savedPos ? "no-spoilers" : "no-spoilers",
      generateImage: true,
    });
    if (res) {
      setAiScenes(res.scenes);
      setAiImageB64(res.imageB64);
    }
  };
  ...
  const sessionForRecap = activeSession
    ? { ...activeSession, endedAt: new Date().toISOString(), endChapter: chapter?.chapterNumber ?? 1, scenesUnlocked: visibleScenes.length, durationMinutes: Math.round((Date.now() - new Date(activeSession.startedAt).getTime()) / 60000) }
    : null;
  ...
  return (
    ...
    >
        ...
        <View style={styles.headerTop}>
          ...
          <View style={styles.headerActions}>
            <TouchableOpacity
              ...
              onPress={() => router.push(`/ambient-companion?bookId=${bookId}&chapterId=${chapter.id}`)}
            >
      ...
      {activeSession && (
        <TouchableOpacity
          style={[styles.sessionBanner, { backgroundColor: colors.primary + "20", borderColor: colors.primary + "40" }]}
      ...
      {/* Progress: scenes unlocked */}
      {savedPos && (
        <View style={[styles.unlockBanner, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      <View style={[styles.modeTabs, { backgroundColor: colors.card, borderColor: colors.border }]}>
        {(["scenes", "characters", "ambient"] as ViewMode[]).map((mode) => (
          <TouchableOpacity
            key={mode}
            style={[styles.modeTab, { backgroundColor: viewMode === mode ? colors.primary : "transparent" }]}
          ...
          >
            ...
            />
            <Text style={[styles.modeTabText, { color: viewMode === mode ? colors.primaryForeground : colors.mutedForeground }]}>
      ...
      {viewMode === "scenes" && (
        <View style={styles.scenesSection}>
          <View style={styles.sectionHeader}>
            ...
            {canGenerateAI && (
              <TouchableOpacity
                style={[styles.aiBtn, { backgroundColor: colors.accent + "20", borderColor: colors.accent + "40" }]}
          ...
          {aiScenes && aiScenes.length > 0 && (
            <View style={styles.aiSection}>
              <View style={[styles.aiBadge, { backgroundColor: colors.accent + "20" }]}>
                <Feather name="sparkles" size={12} color={colors.accent} />
                <Text style={[styles.aiBadgeText, { color: colors.accent }]}>AI Generated Scenes</Text>
              </View>
              {aiScenes.map((scene, i) => (
                <SceneCard
                  ...
                  scene={scene as any}
                  index={i}
                  imageB64={i === 0 ? aiImageB64 : null}
                  onImmersion={() => router.push(`/immersion-mode?bookId=${bookId}&chapterId=${chapter.id}&sceneIndex=${i}`)}
                  onSave={() => {}}
                />
              ))}
            </View>
      ...
      {viewMode === "characters" && (
        <View style={styles.scenesSection}>
          ...
          <ContextualCharacters bookId={bookId} activeCharacterNames={allCharacters} />
        </View>
      )}
      ...
      {viewMode === "ambient" && (
        <View style={styles.scenesSection}>
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Ambient Mode</Text>
          <Text style={[styles.ambientDesc, { color: colors.mutedForeground }]}>
            A full-screen cinematic experience that runs quietly beside your reading session. Tap to enter — swipe left and right to move through scenes.
          </Text>
          <TouchableOpacity
          ...
          <Text style={[styles.ambientHint, { color: colors.mutedForeground }]}>
            Keeps your phone screen on while you read or listen. Soundscape suggestions included.
          </Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  headerAction: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 12, paddingVertical: 7, borderRadius: 20 },
  ...
  sessionBanner: { flexDirection: "row", alignItems: "center", gap: 8, marginHorizontal: 20, marginTop: 12, padding: 12, borderRadius: 12, borderWidth: 1 },
  sessionDot: { width: 8, height: 8, borderRadius: 4 },
  sessionText: { flex: 1, fontSize: 13, fontFamily: "Inter_500Medium" },
  unlockBanner: { flexDirection: "row", alignItems: "center", gap: 6, marginHorizontal: 20, marginTop: 12, padding: 10, borderRadius: 10, borderWidth: 1 },
  unlockText: { fontSize: 13, fontFamily: "Inter_500Medium" },
  unlockHint: { fontSize: 12, fontFamily: "Inter_400Regular" },
  summaryCard: { margin: 20, marginBottom: 0, borderRadius: 16, borderWidth: 1, padding: 16, gap: 8 },
  ...
  modeTabs: { flexDirection: "row", margin: 20, marginBottom: 0, borderRadius: 14, borderWidth: 1, padding: 4, gap: 4 },
  modeTab: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 5, paddingVertical: 9, borderRadius: 10 },
  ...
  scenesSection: { paddingHorizontal: 20, paddingTop: 20, gap: 10 },
  sectionHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  sectionTitle: { fontSize: 20, fontFamily: "Inter_700Bold" },
  aiBtn: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 12, paddingVertical: 7, borderRadius: 12, borderWidth: 1 },
  aiBtnText: { fontSize: 12, fontFamily: "Inter_600SemiBold" },
  aiSection: { gap: 10 },
  aiBadge: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12, alignSelf: "flex-start" },
  aiBadgeText: { fontSize: 12, fontFamily: "Inter_600SemiBold" },
  ambientDesc: { fontSize: 14, fontFamily: "Inter_400Regular", lineHeight: 22 },
  ambientBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 14, borderRadius: 14, marginTop: 4 },
